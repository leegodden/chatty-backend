...
const UserCache = (): UserCache => {
  ...
  const saveUserToCache = async (key: string, userUId: string, createdUser: IUserDocument): Promise<void> => {
  ...
  const getUserFromCache = async (userId: string): Promise<IUserDocument> => {
    try {
      if (!client.isOpen) {
        await client.connect();
      }
